...
type Harness interface {
	...
	// ShouldSkip tells whether a test should be skipped, given its name.
	// If it should be skipped a reason can be returned as well.
	ShouldSkip(testName string) (bool, string)
	...
	// AckBatcherMaker returns a function that makes batchers that can
	// make batches of acks. Most provider tests can return nil for this to get the
	// default ack batcher maker.
	AckBatcherMaker() func(context.Context, *pubsub.Subscription, driver.Subscription) driver.Batcher
}
...
func testSendReceiveTwo(t *testing.T, newHarness HarnessMaker) {
	...
	for i := 0; i < 2; i++ {
		...
		defer cleanup()
		s := pubsub.NewSubscription(ds, h.AckBatcherMaker())
