...
func (v verifyAs) ErrorCheck(k *secrets.Keeper, err error) error {
	...
	if e.ErrorCode() != kmsv1.ErrCodeInvalidCiphertextException {
		return fmt.Errorf("got %q, want %q", e.ErrorCode(), kmsv1.ErrCodeInvalidCiphertextException)
	}
	return nil
}
...
func TestOpenKeeper(t *testing.T) {
	tests := []struct {
		...
		// OK, by alias.
		{"awskmsv2://alias/my-key", false},
		// OK, by ARN with empty Host.
		{"awskmsv2:///arn:aws:kms:us-east-1:932528106278:alias/gocloud-test", false},
		// OK, by ARN with empty Host.
		{"awskmsv2:///arn:aws:kms:us-east-1:932528106278:key/8be0dcc5-da0a-4164-a99f-649015e344b5", false},
		// OK, overriding region.
		{"awskmsv2://alias/my-key?region=us-west1", false},
		// Unknown parameter.
		{"awskmsv2://alias/my-key?param=value", true},
	}
