...
resource "google_sql_database" "guestbook" {
  ...
  provisioner "local-exec" {
    ...
    command = "go run '${path.module}'/../gcp/provision_db/main.go -project='${google_sql_database_instance.guestbook.project}' -service_account='${google_service_account.db_access.email}' -instance='${local.sql_instance}' -database=guestbook -password='${google_sql_user.root.password}' -schema='${path.module}'/../schema.sql"
  }
}
